...
# %%
#FROM GENRE DISTRIBUTION STACKED BARCHART
...
#list of actors defined in bubblechart section

#filter the dataset to include only movies that involve at least one of the actors of interest
filtered_data = df[df['actors'].apply(lambda x: isinstance(x, str) and any(actor in x for actor in actors_of_interest))] #may cause issues with other graphs??
...
app.layout = html.Div([
                ...
                'data': [
                    {
                        ...
                        'marker': {
                            ...
                            'sizemode': 'area',
                            'sizeref': 2. * max(actor_ratings_df['movie_count']) / (10. ** 2),  # adjust the reference size as needed
                        },
    ...
    #boxplot
    html.Div(style={'backgroundColor': 'rgba(255, 255, 255, 0.5)'}, children=[
        html.H1(
    ...
    html.Div([
    html.Div([
        html.H2("Description of the Selected Dataset", style={'textAlign': 'center'}),
    ...
    #datatable
    #html.Div("Insert dataset here", style={'textAlign': 'center'})
])
...
def update_boxplot(selected_genres):
    ...
    fig.update_layout(
        ...
        paper_bgcolor='rgba(255, 255, 255, 0.001)',
